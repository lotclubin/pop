...
def process_pdf_job(local_id, temp_path):
    ...
    try:
        ...
        with open(temp_path, "rb") as f:
            files = {"file": f}
            resp = http_requests.post(JOB_URL, headers=headers, data=data, files=files)
        ...
        # 2. Poll for results
        while True:
            poll_resp = http_requests.get(f"{JOB_URL}/{paddle_job_id}", headers=headers)
            ...
            if state == "pending":
                ...
                job["status"] = "failed"
                job["error"] = f"PaddleOCR failed: {error_msg}"
                return
        ...
        jsonl_resp = http_requests.get(jsonl_url)
                    ...
                    if "markdown" in page_res and "images" in page_res["markdown"]:
                        ...
                        for img_path, img_url in page_res["markdown"]["images"].items():
                            try:
                                ...
                                img_bytes = http_requests.get(img_url, timeout=30).content
