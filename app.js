...
(function () {
  ...
  //  RESYNC
  // ================================
  function bindResyncEvents() {
    if (!resyncBtn || !resyncOkBtn) return;
    
    resyncBtn.addEventListener('click', () => {
      const q = questions[currentIndex];
      if (!q || !rawMarkdown) {
        alert('Resync requires the raw markdown/text. Please upload a structured file or wait for full OCR.');
        return;
      }
      
      const originalBtnText = resyncBtn.innerHTML;
      resyncBtn.innerHTML = 'Syncing...';
      resyncBtn.classList.add('syncing');
      
      setTimeout(() => {
        try {
          const resyncedQ = ExamParser.resyncQuestion(rawMarkdown, q.id);
          if (resyncedQ) {
            // Keep original correctly assigned subjects/types
            resyncedQ.subject = q.subject;
            resyncedQ.type = q.type;
            
            // Re-apply options answers matching logically if possible, or clear them
            answers[currentIndex] = undefined; 
            
            questions[currentIndex] = resyncedQ;
            renderQuestion();
          } else {
            alert('Could not locate a better version of Q' + q.id + ' in the raw text.');
          }
        } catch (err) {
          console.error(err);
          alert('Error during resync: ' + err.message);
        } finally {
          resyncBtn.innerHTML = originalBtnText;
          resyncBtn.classList.remove('syncing');
        }
      }, 300); // small delay to allow UI to update to 'Syncing...'
    });

    resyncOkBtn.addEventListener('click', () => {
      const q = questions[currentIndex];
      if (q) {
        q.resyncConfirmed = true;
        renderQuestion();
      }
    });
  }
  ...
  async function handlePdfUpload(file) {
    showError('');
    showProcessing('Uploading PDF to PaddleOCR...', 'This may take a few minutes depending on file size');
    try {
      ...
      while (true) {
        ...
        else if (statusData.status === 'failed') { hideProcessing(); showError('PaddleOCR failed: ' + (statusData.error || 'Unknown')); return; }
  ...
  function finishProcessing(result, skipSave) {
    ...
    rawMarkdown = result.rawMarkdown || rawMarkdown || '';
  ...
  function detectDirectQuestions(rawJson) {
    ...
    if (rawJson.questions && Array.isArray(rawJson.questions) && rawJson.questions.length > 0) {
      ...
      if (q.text !== undefined || q.question !== undefined || q.options !== undefined) {
        ...
        return rawJson.questions.map(q => {
  ...
  function showError(msg) { uploadError.textContent = msg; uploadError.style.display = msg ? 'block' : 'none'; }
  ...
  function renderQuestion() {
    ...
    // Resync state
    qCard.classList.toggle('resynced', !!q.resynced && !q.resyncConfirmed);
    qCard.classList.toggle('resync-ok', !!q.resyncConfirmed);
    resyncOkBtn.classList.toggle('hidden', !q.resynced || !!q.resyncConfirmed);
  ...
  function renderResults() {
    ...
    const maxScore = activeTemplate && activeTemplate.totalMarks
