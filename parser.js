...
const ExamParser = (function () {
  ...
  function parseQuestionsLineByLine(content, subject, imageMap) {
    ...
    function tryParseQStart(line) {
      ...
      if (/Video\s*Solution/i.test(trimmed)) return null;

      // 1. Try Q-prefixed: Q1 text, Q.3) text, Q3. text
      let m = RE_QSTART.exec(trimmed);
      if (m) {
    ...
    function extractOptLabelFromDiv(line, hasExistingOptions) {
      ...
      // Match <div ...>(A) some text</div> — label with inline text
      const mParensText = trimmed.match(/^<div[^>]*>\s*\(\s*([A-Da-d])\s*\)\s*(.*?)\s*<\/div>$/i);
      if (mParensText) {
      ...
      if (hasExistingOptions) {
        const mBare = trimmed.match(/^<div[^>]*>\s*([A-Da-d])\s*<\/div>$/i);
        if (mBare) {
    ...
    function tryParseOpt(line) {
      const trimmed = line.trim();
      // Skip empty/whitespace lines
      if (!trimmed) return null;

      // 1. Two-col numeric: 'A  3043 J (2) 3024 J'
      let m = RE_TWO_COL_NUM.exec(trimmed);
      if (m) {
      ...
      const allMatches = [];
      let om;
      while ((om = RE_ALL_OPTS.exec(trimmed)) !== null) {
        const lbl = OPT_MAP[om[1]] || om[1].toUpperCase();
        const txt = (om[2] || '').trim();
        if ('ABCD'.includes(lbl)) {
      ...
      const divOptMatch = trimmed.match(/^<div[^>]*>\s*\(?\s*([A-Da-d])\s*\)?\s*<\/div>$/i);
      if (divOptMatch) {
    ...
    function splitPreOptionImages(currentQuestion) {
      ...
      if (sortedByArea.length > 0) {
        ...
        const isDominant =
          largest.meta.width >= 320 &&
          largest.meta.area >= 70000 &&
      ...
      if (current !== null) {
        ...
        if (lastOptionLabel && current.options[lastOptionLabel] !== undefined) {
          ...
          const stripped = line
            ...
            .replace(/!\[[^\]]*\]\([^)]+\)/g, '')
            .trim();
          if (stripped && !tryParseQStart(line) && !tryParseOpt(line)) {
        ...
        const optResult = tryParseOpt(line);
        if (optResult) {
          lastOptionLabel = null;
    ...
    return deduped.map(q => {
      ...
      if (preOptionImages.length > 0) {
        ...
        if (mostlyTextualOptions) {
          ...
          const remainingOptionImgs = split.optionImages.slice();
      ...
      const isNumerical = options.length < 2;
